...
/**
 * Verify the read access authorization to remote repository with pull dry-run.
 *
 * @param {String} repositoryUrl The remote repository URL.
 ...
 * @throws {Error} if not authorized to push.
 */
export async function verifyPull(repositoryUrl, branch, execaOptions) {
  try {
    await execa("git", ["pull", "--dry-run", "--no-verify", repositoryUrl, `HEAD:${branch}`], execaOptions);
  } catch (error) {
    debug(error);
    throw error;
  }
