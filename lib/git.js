...
/**
 * Verify the read access authorization to remote repository with fetch dry-run.
 *
 * @param {String} repositoryUrl The remote repository URL.
 * @param {String} branch The repository branch for which to verify read access.
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 * @throws {Error} if not authorized to fetch.
 */
export async function verifyFetch(repositoryUrl, branch, execaOptions) {
  try {
    await execa("git", ["fetch", "--dry-run", repositoryUrl], execaOptions);
  } catch (error) {
    debug(error);
    throw error;
  }
